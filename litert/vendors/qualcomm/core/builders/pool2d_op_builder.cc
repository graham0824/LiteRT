...
std::vector<OpWrapper> BuildPool2dOp(
    IrPool<TensorWrapper>& tensor_pool, const char* op_type,
    const char* filter_param_name, const char* stride_param_name,
    const char* padding_param_name, const std::vector<TensorWrapperRef>& inputs,
  ...
  TensorWrapper& filter_tensor = tensor_pool.Emplace();
  CreateStaticTensor(
      filter_tensor, "", QNN_DATATYPE_UINT_32, QuantizeParamsWrapperVariant{},
      filter_shape,
  ...
  TensorWrapper& stride_tensor = tensor_pool.Emplace();
  CreateStaticTensor(
  ...
  const std::array<std::uint32_t, 4> padding_data{
      ...
      padding_after_width};
  TensorWrapper& padding_tensor = tensor_pool.Emplace();
  CreateStaticTensor(
