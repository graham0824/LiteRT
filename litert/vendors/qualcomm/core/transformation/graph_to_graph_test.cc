...
TEST(MHAOptimization, Gemma3Prefill) {
  ...
  auto& matmulk0_output = tensor_pool.CreateNativeTensor(
      QNN_DATATYPE_SFIXED_POINT_16, quant_param, {1, 1, 512, 1280});
  op_wrappers.emplace_back(CreateMatmulOp(reshape0_output, kv_cache_k,
  ...
  op_wrappers.emplace_back(CreateMatmulOp(reshape0_output, kv_slice_k,
  ...
  // Slice0
  static constexpr std::array<int32_t, 12> slice0_ranges_data{
      0, 1, 1, 0, 1, 1, 0, 512, 1, 0, 1280, 1};
  auto& slice0_rangs = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_INT_32, {}, {4, 3},
      sizeof(slice0_ranges_data[0]) * slice0_ranges_data.size(),
      slice0_ranges_data.data());
  auto& slice0_output =
      tensor_pool.CloneNativeTensorFrom(reshape2_output, {1, 1, 512, 1280});
  op_wrappers.emplace_back(
      CreateSliceOp(softmax_output, slice0_output, slice0_rangs));
  // Slice1
  static constexpr std::array<int32_t, 12> slice1_ranges_data{
      0, 1, 1, 0, 1, 1, 0, 512, 1, 1280, 1408, 1};
  auto& slice1_rangs = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_INT_32, {}, {4, 3},
      sizeof(slice1_ranges_data[0]) * slice1_ranges_data.size(),
      slice1_ranges_data.data());
  auto& slice1_output =
  ...
  op_wrappers.emplace_back(CreateReshapeOp(transpose1_output, reshape4_output));
...
TEST(MaskTransformTest, Gemma3Mask) {
  ...
  op_wrappers.emplace_back(
      CreateElementWiseNotOp(pattern_input, logic_not_output));
...
TEST_P(GQAGemma4BPrefillTest, GQAGemma4BPrefill) {
  ...
  auto& k_slice_raw = tensor_pool.CreateNativeTensor(
      QNN_DATATYPE_FLOAT_16, quant_param, {1, 2, 128, 256});
  ...
  // Optional KSliceAttnUpdateConvert between far-away Convert output and
  // MatMul1
  ...
  // Optional global mask: Concat local+global masks then Reshape before
  // MaskAdd. When absent, a single mask tensor is used directly.
  TensorWrapper* mask_for_add = nullptr;
  if (params.has_global_mask) {
    auto& local_mask = tensor_pool.CreateNativeTensor(
        QNN_DATATYPE_SFIXED_POINT_8, quant_param, {1, 1, 512, 2688});
    auto& global_mask = tensor_pool.CreateNativeTensor(
        QNN_DATATYPE_SFIXED_POINT_8, quant_param, {1, 1, 512, 2688});
    auto& mask_concat_output =
        tensor_pool.CloneNativeTensorFrom(local_mask, {1, 2, 512, 2688});
    op_wrappers.emplace_back(CreateConcatenationOp({local_mask, global_mask},
                                                   mask_concat_output, 1));
    auto& mask_reshape_output = tensor_pool.CloneNativeTensorFrom(
        mask_concat_output, {1, 2, 512, 2688});
    op_wrappers.emplace_back(
  ...
  // Slice1
  static constexpr std::array<int32_t, 12> slice1_ranges_data{
      0, 1, 1, 0, 2, 1, 0, 512, 1, 2560, 2688, 1};
  auto& slice1_ranges = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_INT_32, {}, {4, 3},
      sizeof(slice1_ranges_data[0]) * slice1_ranges_data.size(),
      slice1_ranges_data.data());
  ...
  auto& in2_base = tensor_pool.CreateNativeTensor(
  ...
  const size_t kNumOpInSha = 11 +
                             (params.has_k_slice_attn_update_convert ? 1 : 0) +
                             (params.has_v_slice_attn_update_convert ? 1 : 0);
  ...
  ASSERT_TRUE(
      op_wrappers[0].IsOpCode(QnnOpCode::kConvert));  // far-away Convert
  if (params.has_global_mask) {
  ...
  ASSERT_TRUE(
      op_wrappers[kPreShaBase + 0].IsOpCode(QnnOpCode::kUnPack));  // UnpackQ
  ASSERT_TRUE(op_wrappers[kPreShaBase + 1].IsOpCode(
      QnnOpCode::kUnPack));  // UnpackK_cache
  ASSERT_TRUE(op_wrappers[kPreShaBase + 2].IsOpCode(
      QnnOpCode::kUnPack));  // UnpackK_slice
  ASSERT_TRUE(op_wrappers[kPreShaBase + 3].IsOpCode(
      QnnOpCode::kUnPack));  // UnpackV_cache
  ASSERT_TRUE(op_wrappers[kPreShaBase + 4].IsOpCode(
      QnnOpCode::kUnPack));  // UnpackV_slice
  ASSERT_TRUE(op_wrappers[kPreShaBase + 5].IsOpCode(
      QnnOpCode::kReshape));  // Reshape(mask)
  ASSERT_TRUE(
      op_wrappers[kPreShaBase + 6].IsOpCode(QnnOpCode::kUnPack));  // UnpackMask
  ...
  for (size_t i = 0; i < kNumHead; ++i) {
    ...
    if (params.has_v_slice_attn_update_convert) {
      ...
      ASSERT_TRUE(
          op_wrappers[(kShaInitIdx + 8 + kShift) + kNumOpInSha * i].IsOpCode(
              QnnOpCode::kConvert));
    }

    // Softmax is at position 4 + shift within each SHA head
    ASSERT_TRUE(
        op_wrappers[(kShaInitIdx + 4 + kShift) + kNumOpInSha * i].IsOpCode(
            QnnOpCode::kSoftmax));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(GQAGemma4BPrefillParams{false, false, false},
    ...
    [](const ::testing::TestParamInfo<GQAGemma4BPrefillParams>& info) {
      if (info.param.has_k_slice_attn_update_convert)
        return "KSliceAttnUpdateConvert";
      if (info.param.has_v_slice_attn_update_convert)
        return "VSliceAttnUpdateConvert";
...
TEST(MHASHATest, FastVlm) {
  ...
  op_wrappers.emplace_back(CreateMatmulOp(reshape0_output, add0_output,
                                          matmul1_output, false, true));
  ...
  // Slice1
  static constexpr std::array<int32_t, 12> slice1_ranges_data{
      0, 1, 1, 0, 2, 1, 0, 896, 1, 1280, 1408, 1};
  auto& slice1_ranges = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_INT_32, {}, {4, 3},
      sizeof(slice1_ranges_data[0]) * slice1_ranges_data.size(),
      slice1_ranges_data.data());
  ...
  auto& matmul3_output = tensor_pool.CreateNativeTensor(
      QNN_DATATYPE_SFIXED_POINT_16, quant_param, {1, 2, 896, 64});
  op_wrappers.emplace_back(CreateMatmulOp(slice1_output, transpose0_output,
                                          matmul3_output, false, true));
...
TEST(MHAOptimization, AttentionWithSelect) {
  ...
  op_wrappers.emplace_back(CreateMatmulOp(transpose_v0_matmul, softmax_matmul,
