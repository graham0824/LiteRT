...
void GraphToGraphTransform(G2GConfig g2g_option, std::vector<OpWrapper>& ops,
  ...
  const std::vector<QnnOpCode> gemma3_mask = {
      QnnOpCode::kElementWiseNot,  // Not
      QnnOpCode::kCast,
      QnnOpCode::kQuantize,
      QnnOpCode::kElementWiseMultiply,  // Mul
  };
  ...
  auto build_gqa_gemma4b_prefill_pattern =
      [](bool has_k_slice_attn_update_convert,
         bool has_v_slice_attn_update_convert, bool has_global_mask) {
        ...
        p.insert(p.end(), {
                              QnnOpCode::kElementWiseAdd,  // mask add
                              QnnOpCode::kSoftmax,
                              QnnOpCode::kStridedSlice,
                              QnnOpCode::kStridedSlice,
        ...
        p.insert(p.end(), {
                              ...
                              QnnOpCode::kElementWiseAdd,  // qkv add
  ...
  for (auto has_k_slice_convert : {false, true}) {
    for (auto has_v_slice_convert : {false, true}) {
      Transform(validate_op_config, ops, tensor_pool,
                build_gqa_gemma4b_prefill_pattern(has_k_slice_convert,
                                                  has_v_slice_convert, false),
