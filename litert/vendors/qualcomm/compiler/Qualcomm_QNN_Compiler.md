...
| `kLiteRtOpCodeShloComposite` | Supports `RmsNorm` (legalized to `QNN_OP_RMS_NORM`), `GroupNorm` (legalized to `QNN_OP_GROUP_NORM`, `QNN_OP_LAYER_NORM`), and `L2Norm` (legalized to `QNN_OP_L2_NORM`). |
